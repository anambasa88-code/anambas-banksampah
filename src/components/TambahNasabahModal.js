...
export default function TambahNasabahModal({ isOpen, onClose }) {
  ...
  useEffect(() => {
    if (isOpen) setForm({ nama_lengkap: "", jenis_kelamin: "LAKI_LAKI", nik: "" });
  }, [isOpen]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-gray-200 dark:border-slate-700">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-slate-700 flex justify-between items-center">
          <h2 className="text-xl font-bold text-gray-800 dark:text-white">Tambah Nasabah Baru</h2>
          <button onClick={() => onClose(false)} className="p-1 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Jenis Kelamin</label>
            <select
              className="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg dark:bg-slate-800 outline-none focus:ring-2 focus:ring-green-500"
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-2 border rounded-lg dark:bg-slate-800 outline-none focus:ring-2 focus:ring-green-500 ${
                form.nik && form.nik.length !== 16 ? "border-red-500" : "border-gray-300 dark:border-slate-600"
              }`}
              value={form.nik}
              onChange={(e) => setForm({ ...form, nik: e.target.value.replace(/\D/g, "") })}
            />
            {form.nik && form.nik.length > 0 && form.nik.length !== 16 && (
              <p className="mt-1 text-xs text-red-500">NIK harus tepat 16 digit (Saat ini: {form.nik.length})</p>
            )}
          </div>
